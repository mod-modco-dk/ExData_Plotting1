##
#  household_power_consumption.txt and plot4.R must be in R's working drirectory
##
...
par(mfrow=c(2,2))
with(df_subset,{
  plot(df_subset$DateTime,df_subset$Global_active_power,type="l",ylab="Global Active Power",xlab="")
  plot(df_subset$DateTime,df_subset$Voltage,type="l",ylab="Voltage",xlab="datetime")
  {
    ...
    legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("black","red","blue"),lty=1,cex=0.4,bty="n")  ## cex to set size 
  }
  plot(df_subset$DateTime,df_subset$Global_reactive_power,type="l",ylab="Global_reactive_power",xlab="datetime")
})
